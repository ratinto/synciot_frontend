...
export const AddRobot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await robotService.addRobot(formData);
      if (response?.success) {
        navigate('/dashboard');
      }
    } catch (err) {
      console.error('Error adding robot:', err);
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div className="px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Add New Robot</h1>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Create a new robot in the system</p>
            </div>
          </div>
        </div>
      </header>
